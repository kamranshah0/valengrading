    ...
    <div class="max-w-4xl mx-auto">
            ...
            <div class="flex items-center justify-between relative">
                ...
                @foreach(['Submission Type', 'Service Level', 'Card Details', 'Shipping', 'Review', 'Payment'] as $index => $step)
                    <div class="flex flex-col items-center w-24">
                        ...
                        <span class="text-xs font-medium text-center leading-tight mt-1 {{ $index + 1 <= 5 ? 'text-white' : 'text-gray-500' }}">{{ $step }}</span>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="mb-8 block md:hidden">
            <div class="flex items-start justify-between relative">
                ...
                <!-- Active Line -->
                <div class="absolute left-[8.33%] top-3 h-[2px] bg-[#A3050A] -z-10 transition-all duration-300" style="width: {{ (5 - 1) * 20 }}%;"></div>
                
                @foreach(['Type', 'Level', 'Cards', 'Ship', 'Review', 'Pay'] as $index => $step)
                    @php $isCompleted = ($index + 1) < 5; $isActive = ($index + 1) == 5; @endphp
        ...
        <div class="bg-[#232528]/80 backdrop-blur-xl rounded-2xl border border-white/5 p-8 shadow-2xl relative overflow-hidden group">
            ...
            <div class="flex justify-between items-center pt-6 border-t border-white/10">
                <a href="{{ route('submission.step4') }}" class="px-5 py-2.5 md:px-6 md:py-3 text-sm md:text-base rounded-xl bg-white/5 text-gray-300 hover:text-white hover:bg-white/10 transition-all duration-300 font-medium border border-white/5">
                    Back
                </a>
                
                <form action="{{ route('submission.processPayment') }}" method="POST">
                    @csrf
                    <button type="submit" class="group relative px-5 py-2.5 md:px-8 md:py-3 text-sm md:text-base rounded-xl bg-gradient-to-r from-red-600 to-[#A3050A] text-white font-bold shadow-[0_0_20px_rgba(163,5,10,0.3)] hover:shadow-[0_0_30px_rgba(163,5,10,0.5)] transition-all duration-300 hover:scale-[1.02]">
