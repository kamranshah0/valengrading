...
@section('title', 'New Service Level')
...
<div class="w-full">
    <div class="bg-[#232528]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-4 md:p-8 shadow-2xl">
        <form method="POST" action="{{ route('admin.service-levels.store') }}" class="space-y-6">
            ...
            <div class="space-y-2">
                <label for="delivery_time" class="text-sm font-medium text-gray-400">Turnaround Time*</label>
            ...
            <div class="space-y-2">
                ...
                <textarea id="icon" name="icon" rows="2"
                    class="w-full bg-[#15171A] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-red-500 transition-colors"
                    placeholder="e.g., <path d='...'></path>">{{ old('icon') }}</textarea>
...
Expert Grading
Encapsulation">{{ old('features') }}</textarea>
