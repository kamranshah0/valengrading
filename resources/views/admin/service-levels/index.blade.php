...
<div class="space-y-6" x-data="{ search: '' }">
    <div class="bg-[#232528]/80 backdrop-blur-xl border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
        ...
        <!-- Mobile Card View -->
        <div class="md:hidden space-y-8 px-4 pb-4 pt-4">
            @php
                $groupedLevels = $serviceLevels->groupBy(function($level) {
                    return $level->submissionType ? $level->submissionType->name : 'Global / Uncategorized';
                });
            @endphp
            
            @foreach($groupedLevels as $groupName => $levels)
                <div class="space-y-4">
                    <h4 class="text-sm font-bold text-red-500 uppercase tracking-widest border-b border-white/5 pb-2">{{ $groupName }}</h4>
        ...
        <div class="hidden md:block overflow-x-auto pb-8">
            @foreach($groupedLevels as $groupName => $levels)
                <div class="mb-8">
                    <h4 class="px-6 py-3 text-sm font-bold text-red-500 uppercase tracking-widest bg-white/5 border-b border-white/5 border-t">{{ $groupName }}</h4>
